...
options {
    ...
    recursion yes;
    allow-query { trusted_zones; private-networks; };
...
ns1     IN      A       200.1.6.5       ; Cambiar a la IP exacta de este contenedor DNS

; ZONA 1 (Proxy)
proxy   IN      A       200.1.6.2       ; Cambiar a la IP del contenedor Nginx de Z1

; ZONA 2 (DMZ)
webgoat IN      A       10.1.2.5        ; Cambiar a IP de WebGoat
ftp     IN      A       10.1.2.6        ; Cambiar a IP de pure-ftpd anónimo
webdmz  IN      A       10.1.2.7        ; Cambiar a IP Apache

; ZONA 3 (App)
dvwa    IN      A       10.2.3.5
ad      IN      A       10.2.3.6        ; Samba AD Controller
```
...
**P: Hice ping a `webgoat.equipoa.local` y el DNS resolvió la IP bien (`10.1.2.5`), pero al abrir el navegador sale "Página no encontrada". ¿Qué falló?**
